...
func TestWorkStopCarriesWhatTheScreenShows(t *testing.T) {
	cases := []struct {
		...
		{"task by its id alone, as the stream stops it", Request{ID: "1", Kind: TaskStop, Target: "aacpanel",
			Work: &Work{ID: "bqjjajuiv"}}, true},
...
func TestSwitchSaysWhereAndCarriesItsProject(t *testing.T) {
	...
	cases := []struct {
		...
		{"to the console with a window", Request{ID: "1", Kind: SessionSwitch, Target: "aacpanel",
			Switch: &Switch{To: SwitchConsole, Window: true}, Project: project}, true},
		{"to the feed with a window", Request{ID: "1", Kind: SessionSwitch, Target: "aacpanel",
			Switch: &Switch{To: SwitchStream, Window: true}, Project: project}, false},
