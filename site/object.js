...
// [id, name, arm (s: the service, x: the executor, c: the collector), kind, what it does]
const NODES = [
    ["service", "the service", "s", "core", "the container: HTTP, API, streams, rules, pushes, the database"],
    ["exec", "the executor", "x", "core", "on the host: the only thing that acts — twenty-two named actions, and the list is closed"],
    ["agent", "the collector", "c", "core", "on the host: reads /proc and the transcripts, writes the snapshot, never writes anything else"],
    ["pg", "Postgres", "s", "infra", "the panel's database: history, the journal, rules, devices"],
    ["proxy", "docker socket proxy", "s", "infra", "docker for the service, GET only — starting or removing a container this way is impossible"],
    ["edge", "the edge", "s", "infra", "nginx and WireGuard on a VPS, in front of the container"],
    ["phone", "the phone", "s", "infra", "the PWA: one column, the composer under the thumb"],
    ["desk", "the desktop", "s", "infra", "the same page: sessions, the feed and the archive side by side"],
    ["push", "Web Push", "s", "infra", "VAPID pushes to the devices that signed in"],
    ["passkeys", "passkeys", "s", "infra", "sign-in: a passkey enrolled with a one-time code from the machine"],
    ["docker", "docker", "x", "infra", "containers and stacks on the host, reached through docker.sock"],
    ["tmux", "tmux + claude", "x", "infra", "the sessions themselves, each in its own tmux window"],
    ["actions", "named actions", "x", "infra", "the closed list the service may ask for, over a unix socket"],
    ["transcripts", "transcripts", "c", "infra", "the conversations on disk — read on the host, never mounted into the container"],
    ["snapshot", "the snapshot", "c", "infra", "state.json: the machine and its sessions, mounted into the service read-only"],
    ["hist", "history and charts", "s", "f", "rollups by minute and by hour; charts from half an hour to a month"],
    ["alerts", "alerts and rules", "s", "f", "thresholds on the numbers; acknowledged from the screen"],
    ["devices", "devices", "s", "f", "the browsers that signed in, and where the pushes go"],
    ["settings", "settings", "s", "f", "shown, not edited"],
    ["map", "the profile map", "s", "f", "profile → group → project: where sessions may open, with what, in which account"],
    ["contours", "contours", "s", "f", "one claude account per group of projects"],
    ["stacks", "containers by stack", "s", "f", "the stack → containers tree with processor, memory and size on disk"],
    ["logs", "logs", "s", "f", "a container's log, right there"],
    ["updown", "bring up / down", "x", "f", "start, stop, restart a container; a whole stack up and down"],
    ["window", "the window", "x", "f", "a window on the host desktop, opened for a session"],
    ["stop", "stop", "x", "f", "stop the turn, a piece of background work, or a subagent"],
    ["composer", "the composer", "x", "f", "a message from the phone into the session"],
    ["files", "files both ways", "x", "f", "a photo or a document into a session, what it produced back out"],
    ["term", "the terminal", "x", "f", "the session's own tmux, typed into from the page"],
    ["slash", "slash commands", "x", "f", "from a closed list"],
    ["permit", "permission prompts", "x", "f", "the console's three answers, on the page, with “from now on”"],
    ["ask", "questions", "x", "f", "the AskUserQuestion card: an option, your own words, or dismiss"],
    ["journal", "the journal", "x", "f", "everything done on the host: who, when, from which device, how it ended"],
    ["sessions", "sessions", "c", "f", "the live sessions, and how full each context is"],
    ["feed", "the feed", "c", "f", "the conversation as it happens: messages, thinking, tool calls, subagents"],
    ["archive", "session archive", "c", "f", "closed conversations, with resume"],
    ["machine", "the machine", "c", "f", "processor, memory, disks, network, top processes"],
    ["probes", "probes", "c", "f", "checks on ports and external services"],
    ["usage", "usage", "c", "f", "the transcripts broken down: by day, model, tool, account, session"],
].map(([id, name, arm, kind, what], i) => ({ id, name, arm, kind, what, i, near: [] }));
const BY = Object.fromEntries(NODES.map((n) => [n.id, n]));
`service-exec service-agent exec-agent pg-service proxy-service proxy-docker edge-service phone-edge desk-edge push-service
 push-phone passkeys-service docker-exec tmux-exec tmux-agent actions-service actions-exec transcripts-agent transcripts-tmux
 snapshot-agent snapshot-service hist-service hist-pg alerts-service alerts-push devices-service devices-push settings-service
 map-service map-exec contours-map stacks-service stacks-proxy logs-service logs-proxy updown-exec updown-docker window-exec
 stop-exec composer-exec composer-tmux files-exec files-service term-exec term-service slash-exec permit-exec permit-service
 ask-agent ask-exec ask-service journal-exec journal-pg sessions-agent sessions-service feed-agent feed-service archive-agent
 archive-transcripts machine-agent machine-service probes-agent usage-agent usage-transcripts`
    .trim().split(/\s+/).forEach((pair) => { const [a, b] = pair.split("-"); BY[a].near.push(BY[b]); BY[b].near.push(BY[a]); });

const TAU = Math.PI * 2, FLAT = 0.55, ARMS = 3, SPIN = 1.4, SPREAD = 0.22;
...
function sprite(temp, rays) {                              // one radial star, cached by temperature
    ...
    const g = c.getContext("2d"), h = SPR / 2, col = TEMP[temp];
    ...
    rg.addColorStop(.05, rgba([lerp(col[0], 255, .6) | 0, lerp(col[1], 255, .6) | 0, lerp(col[2], 255, .6) | 0], .92));
    rg.addColorStop(.10, rgba(col, .42));
    rg.addColorStop(.24, rgba(col, .085));
    rg.addColorStop(.55, rgba(col, .016));
...
const POOL = 420, pts = [];
{
    const r = prng(20260914);
    for (let i = 0; i < POOL; i++) {
        const n = NODES[i];
        pts.push({
            i, n, u: r(), v: r(), w: r(), ph: r(), m: i % 10,
            sz: n ? (n.kind === "core" ? 2.8 : n.kind === "infra" ? 2.2 : 1.9) + r() * .3 : .4 + Math.pow(r(), 7) * 2.2,
            tm: n ? (n.kind === "core" ? 2 : 3 + ((i * 7) % 3)) : 0, tw: i % 10 === 3 && !n,
            x: 0, y: 0, s: 0, a: 0, t: 2, fx: 0, fy: 0, fs: 0, fa: 0, ft: 2, d: r() * .3,
        });
    }
}
...
const gHit = document.createElementNS(NS, "g"), gTxt = document.createElementNS(NS, "g");
const cap = document.getElementById("object-caption");
const labels = new Map();
svg.id = "object-nodes"; svg.setAttribute("aria-hidden", "true"); svg.append(gHit, gTxt);
for (const n of NODES) {
    const c = document.createElementNS(NS, "circle");
    c.setAttribute("class", "hit"); c.setAttribute("r", 15); c.dataset.i = n.i;
    gHit.appendChild(c); n.hit = c;
    if (n.kind === "core") { const t = document.createElementNS(NS, "text"); gTxt.appendChild(t); labels.set(n, t); }
}
const say = (n) => { if (cap) cap.innerHTML = `<b>${n.name}</b> — ${n.what}`; };
function placeLabels() {
    ...
    for (const n of [...labels.keys(), hover].filter(Boolean)) {
        ...
        const p = pts[n.i];
        t.setAttribute("x", Math.round(p.x)); t.setAttribute("y", Math.round(p.y - p.s * 6 - 6));
        t.setAttribute("class", n === hover ? "on" : ""); t.textContent = n.name;
        seen.add(n);
    }
...
function resize() {
    ...
    view.W = W; view.H = H;
    view.S = Math.min(view.W, view.H); view.cx = view.W / 2; view.cy = view.H / 2;
    ...
    svg.setAttribute("width", view.W); svg.setAttribute("height", view.H);
    gHit.setAttribute("transform", `translate(${view.cx} ${view.cy}) scale(1 ${FLAT})`);
...
function boot() {
    ...
    cur = document.documentElement.dataset.chapter || "hero";
    if (cur === "built") buildGalaxy();
    else if (!RM) setTimeout(() => layers.length || buildGalaxy(), 1200);   // drawn once, while the page is still quiet
